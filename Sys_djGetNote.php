...
if($_GET['TIPO']>'') {
	$integrazioniResult=dbselect('select ID, TIPO, DESCRIZIONE from arc_note where tipo = \''.$_GET['TIPO'].'\' ');
	print('<div class="tableContainer">
				<table class="integrazioni"><caption>'.$integrazioniResult['ROWS'][0]['TIPO']."</caption>
	...
	for ($index = 0; $index < $integrazioniResult['NROWS']; $index++) {
		print("\t".'<tr><td id="INT_'.$integrazioniResult['ROWS'][$index]['ID'].'">'.$integrazioniResult['ROWS'][$index]['DESCRIZIONE'].'</td>' .
				'<td style="padding-right: 20px;"><a onClick="addNote('.$integrazioniResult['ROWS'][$index]['ID'].',\''.$_GET['id'].'\');" >Aggiungi</a></td>' .
				'</tr>'."\n");
	}
	print('</tbody></table></div>');
} else {
	print('<h3>Seleziona la tipologia di Note da aggiungere!</h3>');
}
