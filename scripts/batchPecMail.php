...
try {
    ...
    for ($i = 1; $i <= $MC->Nmsgs; $i++) {
        ...
        $pecFile = TMP_PATH . '/' . sha1($head->message_id) . '.eml';
        $imported = $db->query('SELECT * FROM arc_pratiche_pec WHERE mail_hash = :mail_hash', array(
            ':mail_hash' => sha1($head->message_id)
        ))->fetch();
        if ($imported) {
            if(fileVerify($imported)){
                $logger->info(date('Y-m-d H:i:s') . ' già importata la mail  ' . $head->message_id . ' - ' . $head->subject . ' -> ' . $imported['PEC_ID']);
            } else {
                $logger->warning(date('Y-m-d H:i:s') . ' Risalvata la mail ' . $head->message_id . ' - ' . $head->subject . '  nel file -> ' . $pecFile);
                imap_savebody($mailbox, $pecFile, $i);
            }
        } else {
            $logger->info(date('Y-m-d H:i:s') . ' Salvata la mail ' . $head->message_id . ' - ' . $head->subject . '  nel file -> ' . $pecFile);
            imap_savebody($mailbox, $pecFile, $i);
        }
    }
    $logger->info('Salvataggio mail terminata');
