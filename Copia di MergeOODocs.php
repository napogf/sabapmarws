...
'pr.protuscita as OGG_PROTUSCITA, ' .
'pr.note as OGG_NOTE, ' .
...
//	exit;
	...
	if ($altreDestinazioniResult = dbselect($altreDestinazioniQuery)) {
		for ($index = 0; $index < $altreDestinazioniResult['NROWS']; $index++) {
			$altreDestinazioni .= $altreDestinazioniResult['ROWS'][$index]['NOME_COGNOME'] . "\n" ;
			$altreDestinazioni .= $altreDestinazioniResult['ROWS'][$index]['PER_CONOSCENZA']>' '?$altreDestinazioniResult['ROWS'][$index]['PER_CONOSCENZA'] . "\n":'' ;
			$altreDestinazioni .= $altreDestinazioniResult['ROWS'][$index]['VIA'] . "\n" .
	...
	$recFind['NOME_SOPRINTENDENTE'] = isSet ($soprintendente) ? $soprintendente : 'Nome Soprintendente';
