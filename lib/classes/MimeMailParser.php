...
class MimeMailParser {
	...
	protected function parse() {
		...
		foreach($structure as $part_id) {
			...
			if(!is_null(mailparse_msg_get_part_data($part))){
	...
	public function getAttachments() {
		...
		foreach($this->parts as $part) {
			$disposition = (!$this->getPartContentDisposition($part) and $part['content-type'] == 'application/octet-stream') ?
			'inline' : $this->getPartContentDisposition($part);
			if (in_array($disposition, $dispositions)) {
				$attachments[] = new MimeMailParser_attachment(
				(isSet($part['disposition-filename']) ? $part['disposition-filename'] : $part['content-name']) ,
